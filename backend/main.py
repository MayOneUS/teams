...
class NewFromPledgeHandler(BaseHandler):
  ...
  def _loadPledgeInfo(self, pledge_key, user_token):
    resp = urlfetch.fetch(
        "%s/user-info/%s?auth_token=%s" % (
            PLEDGE_SERVICE_REQ, pledge_key, user_token),
  ...
  def get(self, pledge_key):
    user_token = self.request.get("auth_token")
    team = Team.all().filter('user_token =', user_token).get()
    if team is None:
      user_info = self._loadPledgeInfo(pledge_key, user_token)
  ...
  def post(self, user_token):
    ...
    if team is None:
      ...
      if self._loadPledgeInfo(pledge_key, user_token) is None:
        return self.notfound()
